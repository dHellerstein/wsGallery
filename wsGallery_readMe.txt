...
wsGallery:  ver 1.45  -- quickly  list and view  images, and other files,  in a directory tree
...
I.a  Other means of accesing wsGallery data
  ...
  A compact interface, suitable for use with mobile devices, is wsGallery_smal.php.
  As with wsGalley_simple, wsGallery_small works with information created by wsGallery.php.
  ...
  Note that wsGallery_simple.php, and wsGallery_small.php, demonstrates the uses of wsGallery_get.js and wsGallery_get.php.

  See wsGallery_get.txt, and wsGallery_simple.php or wsGallery_small.php, for the details.

I.b Setup and intallation

  wsGallery is part of the wsurvey. package, and uses several javascript and php libraries packaged with wsurvey.
  However, wsGalleryxxx.zip (eg; wsGallary_v140.zip) is shipped with the necessary libraries and can be used in a standalone mode.

  The basic installation steps:
      * unzip wsGallery_vxxx.zip to its own directory. It will unzip to a wsGallery_vxxx directory, which will contain a number of subdirectories.
   ...
   Note: in the following, we assume you install wsGallery to a ../wsGallery/ directory

 1)  Unzip wsGallery_vxxx (xxx a version code) into its own subdirectory in your web tree.
    * wsGallery is typically unzipped to a wsGallery_vxxx directory. wsGallery_vxxx will contain a several sub directories.
     For example: if you saved wsGallery_v140.zip to /wsGallery, /wsGallery/wsGallery_v140 will be created.
      You can use wsGallery_vxxx in url requests. Or rename wsGallery_vxxx to wsGallery. Or copy its contents to an emtpy wsGallery directory. 
 ...
 2) Set some "tree specifications" in wsGallery/galleries/main/wsGallery_treeList.php.
    For a description of the wsGallery_treeList.php, see wsGallery/data/wsGallery_treeList_original.php or
    wsGallery/wsGallery_readMe.txt
 ...
 4) Assuming wsGallery.php ended up in /www/wsGallery (thus: /www is the root web directory),
...
II.a Specifying trees in a `gallery`
   ...
   And under such a root directory there can be hundreds of subdirectories with hundreds of files -- all of which can be
   automatically accessed by wsGallery.
   ...
   Technical note:
      ...
      For example: for the 'main' gallery: the cache info for 'my_photos' tree would use wsGallery/galleries/main/my_photos
...
II.b Specifying a treelist: details
    ...
    Syntax of a row in wsGallery_treeList.php:
        ...
        treename:  treeRootDir, selBase, description
  ...
  In addition to tree specification rows, you can specify a "default tree" parameter (which MUST start with a period).
        .showTree:  aTreename
  ...
  And you can specify one or more "alternative names" for any of the specified trees
       .altname: aTreename, an alternative name
   These specify alternative strings to show in the menu of trees. You can specify zero, one, or more  .altname: lines.
   Each aTreename MUST match one of the specified treenames.
   The `an alternative name` can be a multi-word string -- with no HTML. It should be short (it is used as a button's text).
  ...
  The _default tree.
     ...
     Example (assuming that d:\www is the root directory for this website, and /wsGallery/wsGallery.php is the full selector):
          ...
          .showTree:
	  .altname: oldphotos, photos from the archive
     ...
     Notes:
        ...
        * If invalid trees (that point to non-existent directories) are specified, wsGallery will exit with an error message
...
II.c. Initializing galleries and trees
  ...
  To do this edit the data/wsGallery_params.php file   -- and set 
   ...
   In this case (requireLockme:1), to allow wsGallery to initialize trees:
     ...
     d) Reload wsGallery.php!

  Note that if you have several galleries, switch to these galleries -- wsGallery reloads and will intialize the selected gallery
  (with logon required each time). 

  After this gallery intitialization you should initialize each of its trees. This is done using wsGallery's admin screen.

   e) Re load wsGallery, and logon as an administrator -- use gear  button in the upper right corner, and then click the logon button.
      And enter the password you specified in wsurvey.adminLogon_params.php. 

     An administrative menu will appear, that lists all of the trees specified for the currently gallery.
     You can close this administrative menu by clicking the blue circle-with-dashes button (next to the gear button).
...
The status screen contains a "manage the directories ..." button. We recommend using it to further initialize
...
II.d. Directory initialization
  ...
  However -- there are some additional actions that can improve peformance.
      In particular: using wsGallery's admin tools to 'further' initialize directories.
      That is: create and save information on the potentially numerous directories that comprise a tree.
  
  What is this information?
      Directory "further initialization" will create a "cache".
  ...
  Why further inititialize?
    ...
    b) wsGallery_simple.php, and wsGallery_small.php, can only be used to view directories that have been initialized.
    c) wsGallery "collections" can only work with directories that have been initialized.

  There are a few drawbacks:
    a) Further initialization can be time consuming. If you have thousands of images spread across hundreds of directories of a tree, it can take hours.
   ...
   There is a case for not further initializing:

    a) If you (the administrator) will never create "collections"
    b) You will never use wsGallery_simple.php or wsGallery_small.php
 ...
 To further initialize directories:
...
And if you want to limit "favorite-lists" saved to the server, you can set the enableFavorites parameter. 
Similarly, the enableNotes parameter is used to enable 'file-notes' and 'keywords'.
...
3) After unzipping wsGalleryxxx.zip,  rename (or whatever) the wsGallery_vxxx directory to wsGallery
